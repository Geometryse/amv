<script lang="ts">
	...
	function draw() {
		...
		for (let i = 0; i < barCount; i++) {
			...
			const countedBins = allBins.length > 4 ? 5 : allBins.length;
			const totalPower =
				allBins.length > 4
					? allBins[0] + allBins[1] + allBins[2] + allBins[3] + allBins[4]
					: allBins.reduce((a, b) => a + b, 0);
			// Math.random() < 0.001 && console.log(totalBins, i);
			// for (let j = lowerIndex; j <= upperIndex; j++) {
			// 	const power = dataArray[j];
			// 	// if (power > 100 - Math.sqrt(barCount * totalBins)) {
			// 		totalPower += power;
			// 		// if (upperIndex - j < 50) countedBins++;
			// 	// }
			// }
